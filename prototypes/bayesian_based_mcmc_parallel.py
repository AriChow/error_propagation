...
class bayesian_MCMC():
	...
	def bayesian_parallel(self, i_path, output):
		...
		def VGG_all_features(names, X):
			...
			if os.path.exists(self.data_loc + 'features/bayesian_parallel/VGG_' + self.data_name + '.npz'):
				f = np.load(open(self.data_loc + 'features/bayesian_parallel/VGG_' + self.data_name + '.npz', 'rb'))
				return f.f.arr_0[X, :]
			else:
				f = CNN_all_features(names, 'VGG')
				np.savez(open(self.data_loc + 'features/bayesian_parallel/VGG_' + self.data_name + '.npz', 'wb'), f)
				return f[X, :]

		def inception_all_features(names, X):
			home = os.path.expanduser('~')
			if os.path.exists(self.data_loc + 'features/bayesian_parallel/inception_' + self.data_name + '.npz'):
				f = np.load(open(self.data_loc + 'features/bayesian_parallel/inception_' + self.data_name + '.npz', 'rb'))
				return f.f.arr_0[X, :]
			else:
				f = CNN_all_features(names, 'inception')
				np.savez(open(self.data_loc + 'features/bayesian_parallel/inception_' + self.data_name + '.npz', 'wb'), f)
				return f[X, :]
	...
	def bayesianmcmc(self):
		...
		for z in range(4):
			output = mp.Queue()
			processes = [mp.Process(target=self.bayesian_parallel, args=(i, output)) for i in range(z*3, (z+1)*3)]
