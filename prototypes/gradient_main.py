...
if __name__ == '__main__':
	...
	g.initialize()
	pickle.dump(g, open(results_home + 'intermediate/initial_gradients_all_' + dataset + '.pkl', 'wb'), -1)
	...
	err = g.get_error()
	pickle.dump(g, open(results_home + 'intermediate/gradients_all_' + dataset + '.pkl', 'wb'), -1)

	# Feature extraction agnostic gradients
	# g = GradientQuantification(pipeline, hyper, dataset, data_home)
	# g.initialize()
	g = pickle.load(open(results_home + 'intermediate/initial_gradients_all_' + dataset + '.pkl', 'rb'))
	err01 = g.get_error()
	errors_f = g.gradient_search(epochs=n_epochs, agnostic='inputs')
	err1 = g.get_error()
	pickle.dump(g, open(results_home + 'intermediate/gradients_feature_extraction_all_' + dataset + '.pkl', 'wb'), -1)
	...
	err2 = g.get_error()
	pickle.dump(g, open(results_home + 'intermediate/gradients_dimensionality_reduction_all_' + dataset + '.pkl', 'wb'), -1)
	...
	err3 = g.get_error()
	pickle.dump(g, open(results_home + 'intermediate/gradients_learning_algorithm_all_' + dataset + '.pkl', 'wb'), -1)
	f = open(results_home + 'experiments/gradient_search_all_' + dataset + '.txt', 'w')
	...
	plt.savefig(results_home + 'experiments/gradient_search_all_' + dataset + '.jpg')
	f.close()
