...
E_totals1 = np.zeros((4, 3))
E_directs1 = np.zeros((4, 3))
E_propagations1 = np.zeros((4, 3))
gammas1 = np.zeros((4, 3))
...
for z in range(len(datasets)):
	...
	la_params1 = list(la_params1)

	cnt = 0
	agnostic_naive_all = np.zeros((1, 4))
	...
	for run in range(1, 2):
		...
		min_la_naive1 = {}
		for p in range(len(la_params1)):
			min_la_naive1[p] = 1000000
				...
				if alg == la:
					if 'naive' in alg:  # naive ->
						...
						if 'naive' not in fe and 'naive' not in dr:
							...
							la_param1 = pi.max_features
							dists = []
							for k in range(len(la_params)):
								dists.append(np.linalg.norm(np.asarray(la_param) - np.asarray(la_params[k])))
							la_ind = np.argmin(dists)
							if err < min_la_naive1[la_ind]:  # agnostic -> naive
								min_la_naive1[la_ind] = err
		...
		s_naive = 0
		for k in min_la_naive1.keys():
			if min_la_naive1[k] < 100000:
				agnostic_naive[3] += min_la_naive1[k]
				s_naive += 1
		agnostic_naive[3] /= s_naive
	...
	for run in range(start, stop):
		...
		for i in range(len(path_pipelines)):
			...
			la_ind = np.argmin(dists)
			dists = []
			for k in range(len(la_params1)):
				dists.append(np.linalg.norm(np.asarray(la_param1) - np.asarray(la_params1[k])))
			la_ind1 = np.argmin(dists)
			p = pipeline['feature_extraction']
			...
			for j in range(len(p)):
				...
				if alg == la:
					...
					if err < min_la[la_ind]:  # agnostic -> naive
						min_la[la_ind] = err
					if err < min_la1[la_ind1]:  # agnostic -> naive
						min_la1[la_ind1] = err
		...
		s = 0
		for k in min_la1.keys():
			if min_la1[k] < 100000:
				agnostic_opt[3] += min_la1[k]
				s += 1
		agnostic_opt[3] /= s
	...
	gammas1[z, :] = gamma_mean
	E_totals1[z, :] = E_total
	E_propagations1[z, :] = E_propagation_mean
	E_directs1[z, :] = E_direct
...
colors = ['b', 'g', 'y']
steps = ['Haralick distance', 'Whitening', 'Number of estimators']
x1 = np.asarray([1, 2, 3])
w = 0.2
d = w * np.ones(3)
x2 = x1 + d
...
E_totals = E_totals1
E_directs = E_directs1
E_propagations = E_propagations1
gammas = gammas1
colors = ['b', 'g', 'y']
steps[-1] = 'Max features'
x1 = np.asarray([1, 2, 3])
w = 0.2
d = w * np.ones(3)
x2 = x1 + d
