...
for z in range(len(datasets)):
	...
	obj = pickle.load(open(results_home + 'intermediate/' + type1 + '/' + type1 + '_matsc_dataset2_run_' +
						...
						if 'naive' in alg:  # naive ->
							...
							if 'naive' not in fe and 'naive' in la:
								if err < opt_naive[1]:  # opt -> naive
									opt_naive[1] = err
								dr_param = (pi.n_neighbors, pi.n_components)
								dists = []
								for k in range(len(dr_params)):
									dists.append(np.linalg.norm(np.asarray(dr_param) - np.asarray(dr_params[k])))
								dr_ind = np.argmin(dists)
...
plt.legend()
plt.autoscale()
plt.savefig(results_home + 'figures/error_propagation_alg_comparison_matsc_dataset2.eps')
plt.close()
_, axs = plt.subplots(nrows=1, ncols=1)
...
plt.legend()
plt.autoscale()
plt.savefig(results_home + 'figures/propagation_factor_alg_comparison_matsc_dataset2'
						   '.eps')
plt.close()
