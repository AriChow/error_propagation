...
class RL_MCMC():
	def __init__(self, data_name, data_loc, pipeline, path_resources, hyper_resources, iters):
		...
		for h in hyper1.keys():
			...
			if r[0] < p:  # pick hyper-parameters randomly
				...
				for i in range(len(hypers)):
					H.append(hypers[h])
				if h in discrete:
					r1 = np.random.choice(self.pipeline[h], size=1, p=errs)
					hyper[h] = r1[0]
				else:
					mu = np.mean(H)
					std = np.std(H)
	...
	def rlMcmc(self):
		...
		for i in range(len(pipelines[0])):
			...
			for j in range(len(pipelines)):
				err.append(pipelines[i][j].get_error())
				p.append(pipelines[i][j])
			pipelines1.append(p)
			err_argmin = np.argmin(err)
			self.best_pipelines.append(p[err_argmin[0]])
			self.potential.append(err[err_argmin[0]])
		...
		best_pipeline = self.best_pipelines[err_argmin[0]]
		best_error = self.potential[err_argmin[0]]
