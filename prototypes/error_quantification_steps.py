...
for z in range(len(datasets)):
	...
	hypers = ['haralick_distance', 'pca_whiten', 'n_components', 'n_neighbors', 'n_estimators', 'max_features', 'svm_gamma',
			  'svm_C']
	...
	for run in range(start, stop):
		for z, type1 in enumerate(types):
			...
			# 	for j in range(len(p)):
			# 		alg = p[j]
			# 		if pipeline1['feature_extraction'] == alg or pipeline1['dimensionality_reduction'] == alg or pipeline1[
	...
	for run in range(start, stop):
		for z, type1 in enumerate(types):
			...
			for i in range(len(pipelines)):
				...
				for j in range(len(p)):
					alg = p[j]
					if pi.feature_extraction == alg:
						if pie < min_fe[j]:
							min_fe[j] = pie
				p = pipeline['dimensionality_reduction']
				for j in range(len(p)):
					alg = p[j]
					if pi.dimensionality_reduction == alg:
						if pie < min_dr[j]:
							min_dr[j] = pie
				p = pipeline['learning_algorithm']
				for j in range(len(p)):
					alg = p[j]
					if pi.learning_algorithm == alg:
						if pie < min_la[j]:
							min_la[j] = pie
	...
	grid_error[z, :] = grid_step_error
	random_error[z, :] = random1_step_error
	bayesian_error[z, :] = bayesian1_step_error
