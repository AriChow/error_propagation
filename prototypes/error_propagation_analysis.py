...
for run in range(start, stop):
	obj = pickle.load(
		open(results_home + 'intermediate/' + type1 + '/' + type1 + '_' + data_name + '_run_' +
			 str(run) + '_full__naive.pkl', 'rb'), encoding='latin1')

	pipelines = obj.pipelines
	min_err = 1000000
	naive_min_err = 1000000
	min_fe = [100000] * len(pipeline['feature_extraction'])
	min_dr = [100000] * len(pipeline['dimensionality_reduction'])
	min_la = [100000] * len(pipeline['learning_algorithm'])
	min_all = [100000] * len(pipeline['all'])
...
plt.legend()
plt.autoscale()
plt.savefig(results_home + 'figures/error_propagation_random_pipeline' + data_name + '.jpg')
plt.close()
...
plt.legend()
plt.autoscale()
plt.savefig(results_home + 'figures/propagation_factor_random_pipeline' + data_name + '.jpg')
plt.close()
