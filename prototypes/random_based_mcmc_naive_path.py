...
class random_MCMC():
	...
	def randomMcmc(self):
		...
		if os.path.exists(self.results_loc + 'intermediate/random_MCMC/' + self.type1 + '_' + self.data_name + '_run_' + str(
			...
			self.last_iter = last_object.last_iter
			self.last_cnt = last_object.last_cnt
		...
		t0 = time.time()
		for z in range(start, len(paths)):
			path = paths[z]
			last_error = 1000000
			objects = []
			error_curves = []
			best_pipelines = []
			if last_object is not None and z == last_object.last_iter:
